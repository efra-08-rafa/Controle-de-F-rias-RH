...
import { lerSessao, temPermissao, SESSION_COOKIE, type Permissao, type Papel } from './lib/server-auth';
...
function respostaApi(status: number, erro: string) {
...
export function proxy(request: NextRequest) {
  ...
  if (pathname === '/api/auth/login' || pathname === '/api/auth/logout' || pathname === '/api/auth/session') return NextResponse.next();
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (permissao && !temPermissao(sessao.papel, permissao)) return respostaApi(403, 'Sem permissão para esta operação.');
  ...
  const permissao = permissaoDaRota(pathname);
  if (permissao && !temPermissao(sessao.papel, permissao)) return NextResponse.redirect(new URL('/', request.url));
