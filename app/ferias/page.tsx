...
function calcularDireito(f: Funcionario) {
  ...
  if (f.tipoContrato === 'Permanente') {
    ...
    return Math.max(0, new Date().getFullYear() - anoInicio + 1) * 30;
  }
...
export default function FeriasPage() {
  ...
  function registarFerias() {
    ...
    setFuncionarios((lista) => lista.map((item) => item.processo !== funcionario.processo ? item : ({ ...item, diasUtilizados: item.diasUtilizados + dias, inicioFerias: inicio, fimFerias: fim, historico: [...(item.historico || []), novoHistorico] })));
    ...
    <main><div className="container">
      ...
      <section className="section card"><h2>Requisitar férias</h2>
        <div className="form-grid">
          <label>Funcionário<select value={processoSelecionado} onChange={(e) => { setProcessoSelecionado(e.target.value); setMensagem(''); setErro(''); }}><option value="">Selecione o funcionário</option>{funcionarios.map((f) => <option key={f.processo} value={f.processo}>{f.processo} — {f.nome}</option>)}</select></label>
          <label>Data de início<input type="date" value={inicio} onChange={(e) => { setInicio(e.target.value); setMensagem(''); setErro(''); }} /></label>
          <label>Quantidade de dias<input type="number" min="1" max={restantes || undefined} value={quantidadeDias} onChange={(e) => { setQuantidadeDias(e.target.value); setMensagem(''); setErro(''); }} placeholder="Ex.: 5" /></label>
        </div>

        {funcionario && <div className="grid" style={{ marginTop: 20 }}><div className="card"><span className="card-label">Departamento</span><strong className="card-value" style={{ fontSize: 20 }}>{funcionario.departamento}</strong></div><div className="card"><span className="card-label">Tipo de contrato</span><strong className="card-value" style={{ fontSize: 20 }}>{funcionario.tipoContrato}</strong></div><div className="card"><span className="card-label">Direito acumulado</span><strong className="card-value">{direito} <small>dias</small></strong></div><div className="card"><span className="card-label">Dias restantes</span><strong className="card-value">{restantes} <small>dias</small></strong></div></div>}

        <div className="card" style={{ marginTop: 16 }}>
          <strong>Resumo automático</strong>
          <p style={{ marginBottom: 6 }}>Dias solicitados: <strong>{quantidadeDias ? quantidadeDias : '—'}</strong></p>
      ...
      <section className="section"><h2>Resumo dos funcionários</h2>{funcionarios.length === 0 ? <p className="empty-state">Nenhum funcionário disponível.</p> : <div className="table-wrapper"><table><thead><tr><th>Processo</th><th>Nome</th><th>Departamento</th><th>Direito acumulado</th><th>Utilizados</th><th>Restantes</th><th>Estado</th></tr></thead><tbody>{funcionarios.map((item) => { const d = calcularDireito(item); const r = Math.max(0, d - item.diasUtilizados); return <tr key={item.processo}><td>{item.processo}</td><td>{item.nome}</td><td>{item.departamento}</td><td>{d} dias</td><td>{item.diasUtilizados} dias</td><td>{r} dias</td><td>{calcularEstado(item.inicioFerias, item.fimFerias)}</td></tr>; })}</tbody></table></div>}</section>

      <section className="section"><h2>Controlo anual de férias</h2><p>O sistema apresenta automaticamente, ano por ano, os dias adquiridos, os dias utilizados nesse ano e o saldo acumulado que transita para o ano seguinte.</p>{funcionarios.length === 0 ? <p className="empty-state">Nenhum funcionário disponível.</p> : <div className="history-list">{funcionarios.map((f) => { const resumo = calcularResumoAnual(f); const totalGanho = resumo.reduce((soma, item) => soma + item.ganhou, 0); const totalUtilizado = resumo.reduce((soma, item) => soma + item.utilizado, 0); const saldoFinal = resumo.length ? resumo[resumo.length - 1].saldoAcumulado : 0; return <div className="card" key={`anual-${f.processo}`}><h3 style={{ marginTop: 0 }}>Processo {f.processo} — {f.nome}</h3><p><strong>{f.tipoContrato}</strong> · {f.departamento} · Direito adquirido: <strong>{totalGanho} dias</strong> · Utilizado: <strong>{totalUtilizado} dias</strong> · Saldo acumulado: <strong>{saldoFinal} dias</strong></p><div className="table-wrapper"><table><thead><tr><th>Ano</th><th>Dias ganhos</th><th>Dias utilizados</th><th>Saldo acumulado</th></tr></thead><tbody>{resumo.map((item) => <tr key={`${f.processo}-${item.ano}`}><td><strong>{item.ano}</strong></td><td>{item.ganhou} dias</td><td>{item.utilizado} dias</td><td><strong>{item.saldoAcumulado} dias</strong></td></tr>)}</tbody></table></div></div>; })}</div>}</section>

      <section className="section"><h2>Histórico de férias</h2>{funcionarios.every((f) => !f.historico?.length) ? <p className="empty-state">O histórico aparecerá aqui quando houver férias registadas.</p> : <div className="table-wrapper"><table><thead><tr><th>Processo</th><th>Funcionário</th><th>Período</th><th>Dias</th><th>Estado</th></tr></thead><tbody>{funcionarios.flatMap((f) => (f.historico || []).map((h, i) => <tr key={`${f.processo}-${i}-${h.inicio}`}><td>{f.processo}</td><td>{f.nome}</td><td>{formatarData(h.inicio)} — {formatarData(h.fim)}</td><td>{h.dias}</td><td>{calcularEstado(h.inicio, h.fim)}</td></tr>))}</tbody></table></div>}</section>

      <section className="section card"><h2>Regras de férias</h2><p><strong>Permanente:</strong> 30 dias de férias por cada ano contabilizado.</p><p><strong>Acumulação:</strong> o saldo não utilizado passa automaticamente para os anos seguintes.</p><p><strong>Início do cálculo:</strong> ano 2000.</p><p><strong>Fim das férias:</strong> calculado automaticamente a partir da data de início e da quantidade de dias.</p></section>
    </div></main>
  );
}
