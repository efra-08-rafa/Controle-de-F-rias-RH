...
function mapear(f: any, hist: Record<string, any[]>): Funcionario {
  return {
    processo: f.processo || '', nome: f.nome || '', tipoContrato: f.tipoContrato,
    admissao: f.admissao || '', fimContrato: f.fimContrato || '', diasUtilizados: 0,
    inicioFerias: '', fimFerias: '', historico: (hist[f.id] || []).map(x => ({
      inicio: x.inicio, fim: x.fim, dias: Number(x.dias) || 0, estado: x.estado || ''
    })),
    id: f.id, contacto: f.contacto || '', departamento: f.departamento || '—'
  };
}
function normalizarDepartamento(s: string) { return s.trim().toLowerCase().replace(/venda$/, 'vendas'); }
...
export default function Home() {
  ...
  async function carregar() {
    ...
    try {
      ...
      const rF = await fetch(`/api/funcionarios?_painel=${stamp}`, { cache: 'no-store' });
      const df = await rF.json();
      if (!rF.ok) throw new Error(df.erro || 'Não foi possível carregar funcionários.');

      const funcionariosApi = Array.isArray(df.funcionarios) ? df.funcionarios : [];
      let hist: Record<string, any[]> = {};
      try {
        const rH = await fetch(`/api/ferias?_painel=${stamp}`, { cache: 'no-store' });
        const dh = await rH.json();
        if (rH.ok) for (const x of dh.ferias || []) (hist[x.funcionarioId] ??= []).push(x);
      } catch { /* férias são complementares; não podem zerar o contador de funcionários */ }

      setFuncionarios(funcionariosApi.map((f: any) => mapear(f, hist)));
    } catch (e) {
      setFuncionarios([]);
  ...
  return <main><div className="container">
    <section className="hero"><h1>Painel de férias dos Recursos Humanos</h1><p>Dados atualizados automaticamente através das APIs do sistema.</p>{erro && <p className="form-message" style={{ color: '#dc2626' }}>{erro}</p>}</section>
    ...
    <section className="section"><h2>Departamentos</h2><div className="department-list">{departamentos.map(dep => {
      const n = funcionarios.filter(f => normalizarDepartamento(f.departamento) === normalizarDepartamento(dep)).length;
      return <a className="department" href={`/departamentos?nome=${encodeURIComponent(dep)}`} key={dep}><strong>{dep}</strong><span>{n} funcionário(s)</span></a>;
    })}</div></section>
    <section className="section"><h2>Alertas automáticos</h2>{!carregado ? <p className="empty-state">A carregar dados...</p> : !funcionarios.length ? <p className="empty-state">Cadastre funcionários para começar a receber alertas automáticos.</p> : <>{alertasVencidas.map(f => <div className="alert-card danger" key={`v-${f.processo}`}><strong>Férias vencidas:</strong> {f.nome} — {diasVencidos(f)} dia(s) de saldo de anos anteriores.</div>)}{alertasProximas.map(f => <div className="alert-card" key={`p-${f.processo}`}><strong>Férias próximas:</strong> {f.nome} — existe um período com início próximo.</div>)}{alertasContrato.map(f => <div className="alert-card warning" key={`c-${f.processo}`}><strong>Contrato a terminar:</strong> {f.nome} — {formatar(f.fimContrato)}.</div>)}{!alertasVencidas.length && !alertasProximas.length && !alertasContrato.length && <p className="empty-state">Nenhum alerta pendente neste momento.</p>}</>}</section>
    <section className="section"><h2>Saldo dos funcionários</h2>{!funcionarios.length ? <p className="empty-state">Nenhum funcionário cadastrado.</p> : <div className="table-wrapper"><table><thead><tr><th>Processo</th><th>Nome</th><th>Direito</th><th>Utilizados</th><th>Saldo</th><th>Estado</th></tr></thead><tbody>{funcionarios.map(f => { const h = f.historico || []; const ultimo = h[h.length - 1]; return <tr key={f.id}><td>{f.processo}</td><td><strong>{f.nome}</strong></td><td>{direitoAcumulado(f)} dias</td><td>{diasUtilizadosReal(f)} dias</td><td><strong>{saldoRestante(f)} dias</strong></td><td>{ultimo ? estadoFerias(ultimo.inicio, ultimo.fim) : 'Disponível'}</td></tr>; })}</tbody></table></div>}</section>
