...
type Envelope = {
  ...
  entidade?: 'funcionario' | 'ferias' | 'utilizador';
  entidadeId?: string;
...
export async function POST(request: Request) {
  try {
    ...
    for (const item of eventos) {
      const operacaoId = item.operacaoId?.trim();
      const dispositivoId = item.dispositivoId?.trim();
      if (!operacaoId || !dispositivoId || !item.entidade || !item.entidadeId || !item.operacao || !Number.isInteger(item.versaoLocal)) {
        resultados.push({operacaoId:operacaoId || null,status:'ERRO',mensagem:'Evento de sincronização inválido.'});
        continue;
      }
      if (item.entidade === 'utilizador' && sessao.papel !== 'Administrador') {
        resultados.push({operacaoId,status:'ERRO',mensagem:'Sem permissão para sincronizar utilizadores.'});
        continue;
      }
      try {
        const result = await withTransaction(async (client) => {
          ...
          if (item.operacao !== 'CREATE' && atual.rows[0] && item.versaoLocal !== versaoServidor) {
            await client.query(`insert into sincronizacao_eventos(dispositivo_id,operacao_id,entidade,entidade_id,operacao,versao_local,recebido_em,processado_em,status,mensagem_erro) values($1,$2,$3,$4,$5,$6,datetime('now'),datetime('now'),'CONFLITO',$7)`,[dispositivoId,operacaoId,item.entidade,item.entidadeId,item.operacao,item.versaoLocal,'Versão local não coincide com a versão do servidor/banco.']);
            return {status:'CONFLITO' as const,versaoServidor};
          }
          await client.query(`insert into sincronizacao_eventos(dispositivo_id,operacao_id,entidade,entidade_id,operacao,versao_local,recebido_em,processado_em,status) values($1,$2,$3,$4,$5,$6,datetime('now'),datetime('now'),'PROCESSADO')`,[dispositivoId,operacaoId,item.entidade,item.entidadeId,item.operacao,item.versaoLocal]);
          return {status:'PROCESSADO' as const,versaoServidor:versaoServidor || undefined};
        });
        resultados.push({operacaoId,...result});
      } catch (error) {
        resultados.push({operacaoId,status:'ERRO',mensagem:error instanceof Error ? error.message : 'Erro de sincronização.'});
      }
    }
