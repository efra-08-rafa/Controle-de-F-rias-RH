...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await dbQuery(`
      select f.id, f.processo, f.nome, f.nuit, f.bi, f.contacto,
        f.departamento_id as "departamentoId", d.nome as departamento, f.cargo,
        f.tipo_contrato as "tipoContrato", f.data_admissao as admissao,
        f.fim_contrato as "fimContrato", f.ativo, f.versao,
        f.criado_em as "criadoEm", f.atualizado_em as "atualizadoEm"
      from funcionarios f left join departamentos d on d.id=f.departamento_id where f.id=$1`, [id]);
    ...
    return NextResponse.json({ ok:false, erro:'Não foi possível carregar o funcionário.' }, { status:500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const dept = await departamentoId(input);

    const funcionario = await withTransaction(async (client) => {
      const atual = await client.query('select * from funcionarios where id=$1 for update', [id]);
      if (!atual.rows[0]) throw new Error('NAO_ENCONTRADO');
      const antigo = atual.rows[0];
      const result = await client.query(
        `update funcionarios set processo=$1,nome=$2,nuit=$3,bi=$4,contacto=$5,departamento_id=$6,cargo=$7,
         tipo_contrato=$8,data_admissao=$9,fim_contrato=$10,atualizado_por=$11,atualizado_em=now(),versao=versao+1 where id=$12
         returning id,processo,nome,nuit,bi,contacto,departamento_id as "departamentoId",cargo,tipo_contrato as "tipoContrato",
         data_admissao as admissao,fim_contrato as "fimContrato",ativo,versao,atualizado_em as "atualizadoEm"`,
        [dados.processo,dados.nome,texto(input.nuit)||null,texto(input.bi)||null,texto(input.contacto)||null,dept,texto(input.cargo)||null,
         dados.tipoContrato,dados.admissao,dados.fimContrato,sessao.utilizadorId,id],
      );
      const novo = result.rows[0];
      await client.query(
        `insert into auditoria(entidade,entidade_id,operacao,utilizador_id,dispositivo_id,origem,antes,depois)
         values('funcionario',$1,'UPDATE',$2,$3,'online',$4::jsonb,$5::jsonb)`,
        [id,sessao.utilizadorId,request.headers.get('x-device-id')||null,JSON.stringify(antigo),JSON.stringify(novo)],
      );
      return novo;
    });
    ...
    return NextResponse.json({ok:false,erro:'Não foi possível atualizar o funcionário.'},{status:500});
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await withTransaction(async (client) => {
      const atual = await client.query('select * from funcionarios where id=$1 for update',[id]);
      if (!atual.rows[0]) throw new Error('NAO_ENCONTRADO');
      await client.query('update funcionarios set ativo=false,atualizado_por=$1,atualizado_em=now(),versao=versao+1 where id=$2',[sessao.utilizadorId,id]);
      await client.query(
        `insert into auditoria(entidade,entidade_id,operacao,utilizador_id,dispositivo_id,origem,antes)
         values('funcionario',$1,'DELETE',$2,$3,'online',$4::jsonb)`,
        [id,sessao.utilizadorId,request.headers.get('x-device-id')||null,JSON.stringify(atual.rows[0])],
      );
    });
    ...
    return NextResponse.json({ok:false,erro:'Não foi possível desativar o funcionário.'},{status:500});
  }
}
