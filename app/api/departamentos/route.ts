...
export async function GET() {
  try {
    ...
    const result = await dbQuery(`select d.id,d.nome,d.ativo,d.criado_em as "criadoEm",d.atualizado_em as "atualizadoEm",count(f.id) as "totalFuncionarios" from departamentos d left join funcionarios f on f.departamento_id=d.id and f.ativo=true where d.ativo=true group by d.id,d.nome,d.ativo,d.criado_em,d.atualizado_em order by d.nome asc`);
...
export async function POST(request: Request) {
  try {
    ...
    const departamento = await withTransaction(async client => {
      ...
      const id = randomUUID();
      const result = await client.query(`insert into departamentos(id,nome,ativo,atualizado_em) values($1,$2,1,CURRENT_TIMESTAMP) returning id,nome,ativo,criado_em as "criadoEm",atualizado_em as "atualizadoEm"`, [id, nome]);
