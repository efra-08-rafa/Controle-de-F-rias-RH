...
CREATE TABLE IF NOT EXISTS funcionarios (id TEXT PRIMARY KEY,processo TEXT NOT NULL UNIQUE,nome TEXT NOT NULL,nuit TEXT,bi TEXT,contacto TEXT,departamento_id TEXT,cargo TEXT,tipo_contrato TEXT NOT NULL CHECK (tipo_contrato IN ('Permanente','Contratado')),data_admissao TEXT NOT NULL,fim_contrato TEXT,ativo INTEGER NOT NULL DEFAULT 1,versao INTEGER NOT NULL DEFAULT 1,criado_por TEXT,atualizado_por TEXT,criado_em TEXT NOT NULL DEFAULT (datetime('now')),atualizado_em TEXT NOT NULL,FOREIGN KEY (departamento_id) REFERENCES departamentos(id));
CREATE TABLE IF NOT EXISTS ferias (id TEXT PRIMARY KEY,funcionario_id TEXT NOT NULL,inicio TEXT NOT NULL,fim TEXT NOT NULL,dias INTEGER NOT NULL CHECK (dias > 0),estado TEXT NOT NULL,versao INTEGER NOT NULL DEFAULT 1,criado_por TEXT,atualizado_por TEXT,criado_em TEXT NOT NULL DEFAULT (datetime('now')),atualizado_em TEXT NOT NULL,FOREIGN KEY (funcionario_id) REFERENCES funcionarios(id) ON DELETE CASCADE);
