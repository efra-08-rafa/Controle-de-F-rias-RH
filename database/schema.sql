-- Schema central do Controlo de Férias RH
-- PostgreSQL. Não contém credenciais nem dados reais.
...
create table if not exists funcionarios (
  ...
  atualizado_por uuid references utilizadores(id),
  criado_em timestamptz not null default now(),
  atualizado_em timestamptz not null default now(),
  constraint contrato_datas_validas check (fim_contrato is null or fim_contrato >= data_admissao)
);
...
create table if not exists ferias (
  ...
  criado_em timestamptz not null default now(),
  atualizado_em timestamptz not null default now(),
  constraint ferias_datas_validas check (fim >= inicio)
);
...
create table if not exists auditoria (
  ...
  entidade_id uuid,
  operacao text not null check (operacao in ('CREATE', 'UPDATE', 'DELETE', 'SYNC')),
  utilizador_id uuid references utilizadores(id),
  dispositivo_id text,
  origem text not null default 'online' check (origem in ('online', 'offline', 'sync')),
  antes jsonb,
  depois jsonb,
  criado_em timestamptz not null default now()
);

create table if not exists sincronizacao_eventos (
  id uuid primary key default gen_random_uuid(),
  ...
  status text not null default 'PENDENTE' check (status in ('PENDENTE', 'PROCESSADO', 'CONFLITO', 'ERRO')),
  mensagem_erro text
);
...
create index if not exists idx_sync_status on sincronizacao_eventos(status);
